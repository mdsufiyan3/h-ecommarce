...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (editIcon) {
        editIcon.addEventListener('click', function() {
            if (editOverlay) {
                ...
                const { stockStatus, sizes } = ProductSync.getProductData(10); // Changed from 1 to 10
    ...
    editForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            await ProductSync.updateProductData(10, formData); // Changed from 1 to 10
    ...
    function updateSizeButtonsDisplay() {
        const savedSizes = JSON.parse(localStorage.getItem('pd10_productSizes')) || ['M', 'L', 'XL']; // Changed from pd3 to pd6
    ...
    function loadSavedState() {
        const productId = 10; // For pd6, changed from pd3
    ...
    if (viewDetailsBtn) {
        viewDetailsBtn.addEventListener('click', function() {
            ...
            window.location.href = `/wintar pd html and js&css file/Wpd10.html?id=${productId}`; // Changed from pd3 to pd6
        });
    }
