...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (editIcon) {
        editIcon.addEventListener('click', function() {
            if (editOverlay) {
                ...
                const { stockStatus, sizes } = ProductSync.getProductData(5);
    ...
    editForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            await ProductSync.updateProductData(5, formData);
    ...
    function updateSizeButtonsDisplay() {
        const savedSizes = JSON.parse(localStorage.getItem('pd5_productSizes')) || ['M', 'L', 'XL']; // Changed from pd6 to pd5
    ...
    function loadSavedState() {
        const productId = 5; // For pd6, changed from pd3
    ...
    if (viewDetailsBtn) {
        viewDetailsBtn.addEventListener('click', function() {
            ...
            window.location.href = `/wintar pd html and js&css file/H.Wpd5.html?id=${productId}`; // Changed from pd6 to pd5
        });
    }
