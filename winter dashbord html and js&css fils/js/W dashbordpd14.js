...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (editIcon) {
        editIcon.addEventListener('click', function() {
            if (editOverlay) {
                ...
                const { stockStatus, sizes } = ProductSync.getProductData(14);  // Changed from 1
    ...
    editForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            await ProductSync.updateProductData(14, formData);  // Changed from 1
    ...
    function updateSizeButtonsDisplay() {
        const savedSizes = JSON.parse(localStorage.getItem('pd14_productSizes')) || ['M', 'L', 'XL']; // Changed from pd3 to pd6
    ...
    function loadSavedState() {
        const productId = 14; // For pd6, changed from pd3
    ...
    if (viewDetailsBtn) {
        viewDetailsBtn.addEventListener('click', function() {
            ...
            window.location.href = `/wintar pd html and js&css file/Wpd14.html?id=${productId}`; // Changed from pd3 to pd6
        });
    }
