...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (editIcon) {
        editIcon.addEventListener('click', function() {
            if (editOverlay) {
                ...
                const { stockStatus, sizes } = ProductSync.getProductData(8);
    ...
    editForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            await ProductSync.updateProductData(8, formData);
    ...
    function updateSizeButtonsDisplay() {
        const savedSizes = JSON.parse(localStorage.getItem('pd8_productSizes')) || ['M', 'L', 'XL']; // Changed from pd6 to pd8
    ...
    function loadSavedState() {
        const productId = 8; // For pd8, changed from pd3
    ...
    if (viewDetailsBtn) {
        viewDetailsBtn.addEventListener('click', function() {
            ...
            window.location.href = `/wintar pd html and js&css file/Wpd8.html?id=${productId}`; // Changed from pd3 to pd6
        });
    }
...
// Update function call with new productId
function updateProductDisplay() {
    const productId = 8; // Changed from 6 to 8
    const { stockStatus, sizes } = ProductSync.getProductData(productId);
...
window.addEventListener('product-updated', (event) => {
    if (event.detail.productId === 8) { // Changed from 6 to 8
