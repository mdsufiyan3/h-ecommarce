...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (editIcon) {
        editIcon.addEventListener('click', function() {
            if (editOverlay) {
                ...
                const { stockStatus, sizes } = ProductSync.getProductData(12);
    ...
    editForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            // Save data using ProductSync - Changed from 1 to 12
            await ProductSync.updateProductData(12, formData);
    ...
    function updateSizeButtonsDisplay() {
        // Changed from pd1 to pd12
        const savedSizes = JSON.parse(localStorage.getItem('pd12_productSizes')) || ['M', 'L', 'XL'];
    ...
    if (viewDetailsBtn) {
        viewDetailsBtn.addEventListener('click', function() {
            ...
            window.location.href = `/wintar pd html and js&css file/Wpd12.html?id=${productId}`; // Changed from pd3 to pd6
        });
    }

    // Update the loadSavedState callf
    try {
        loadSavedState();
    } catch (error) {
